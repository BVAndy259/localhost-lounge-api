...
export async function sendWaiterWhatsapp(waiterId: number, message: string): Promise<{ success: boolean; reason?: string }> {
...
export async function notifyAllWaiters(): Promise<{ total: number; sent: number; failed: { name: string; reason: string }[] }> {
  ...
  const result = { total: waiters.length, sent: 0, failed: [] as { name: string; reason: string }[] };
  ...
  for (const waiter of waiters) {
    ...
    else result.failed.push({ name: waiter.name, reason: r.reason || 'Error' });
  }
