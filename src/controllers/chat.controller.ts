...
export const ChatController = {
  async handleWebMessage(req: Request, res: Response): Promise<Response | void> {
    try {
      ...
      } else if (typeof aiResponse.payload !== 'object' || Array.isArray(aiResponse.payload)) {
        aiResponse.payload = {};
      }
      ...
      if (aiResponse.action === 'SHOW_MENU') {
          ...
          const reservations = await prisma.reservation.findMany({
            where: {
              OR: [
                ...
                ...(hasNumeric ? [{ table: { table_number: numericTerm } }] : []),
                {
                  client: {
                    OR: [
                      {
          ...
          if (isNaN(peopleInt) || peopleInt <= 0) {
            ...
            if (!tableId) {
              ...
              try {
                ...
                aiResponse.payload = {};
                aiResponse.reply =
                  error?.message || 'No pude registrar la reserva. Intenta de nuevo.';
              }
            }
          }
        }
        ...
        if (!resolvedClientId) {
          aiResponse.reply = 'Para mostrar tu historial necesito tu correo o telefono de cliente.';
