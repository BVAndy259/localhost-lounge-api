...
export const WaiterController = {
  ...
  async update(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { name } = req.body;
      if (!name) {
        res.status(400).json({ error: 'Es necesario indicar el nombre del mesero para realizar la actualización' });
        return;
      }
  ...
  async toggleStatus(req: Request, res: Response): Promise<void> {
    try {
      ...
      const statusMessage = active ? 'activated' : 'deactivated/suspended';
