...
export const AuthController = {
    async register(req: Request, res: Response): Promise<void> {
        try { 
            ...
            if (!name || !email || !password) {
                res.status(400).json({error: 'Es necesario introducir el nombre, el correo electrónico y la contraseña'})
                return
            }
    ...
    async login(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!email || !password) {
                res.status(400).json({error: 'Es necesario introducir el correo electrónico y la contraseña'})
                return
            }
