...
async function callOpenRouter(prompt) {
    const apiKey = env_1.env.OPENROUTER_API_KEY;
    if (!apiKey) {
    ...
    for (const model of models) {
        try {
            ...
            const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are an API that ONLY outputs raw, valid JSON. Do not include markdown formatting like ```json. ONLY return the JSON object.',
                        },
                        {
                            role: 'user',
                            content: prompt,
            ...
            if (!response.ok) {
                ...
                logger_1.logger.warn(`[OPENROUTER] Modelo ${model} no disponible, probando el siguiente...`);
                continue;
            }
            ...
            logger_1.logger.warn(`[OPENROUTER] Modelo ${model} dio error de conexión: ${lastError.message}`);
        }
    }
...
exports.AIService = {
    async processClientWebMessage(clientMessage, history, clientData) {
        try {
            const prompt = `
      ...
      `;
            ...
            logger_1.logger.error('[IA ERROR] Copiloto Cliente falló:', error);
            return {
                action: 'REPLY',
    ...
    async processWorkerWebMessage(workerMessage, history, userRole = 'RECEPCIONISTA') {
        try {
            const prompt = `
      ...
      `;
            ...
            logger_1.logger.error('[IA ERROR] Copiloto Trabajador falló:', error);
            return {
                action: 'REPLY',
